...
@property (strong, nonatomic) RedpacketErrorView *retryViw;
...
- (void)initData {
    if ([self.delegate respondsToSelector:@selector(getGroupMemberListCompletionHandle:)]) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [self.delegate getGroupMemberListCompletionHandle:^(NSArray<RPUserInfo *> *groupMemberList) {
                ...
                for (RPUserInfo *userInfo in _userInfoArray) {
                    
                    if (![userInfo.userID isEqualToString:[RPRedpacketAliauth redpacketCurrentUser].userID]) {
                        
                        //专属红包不能包含自己
                        [_dataSource addObject:[NSString stringWithFormat:@"%@%@%@",userInfo.userName,@"-=sorting=-",userInfo.userID]];
