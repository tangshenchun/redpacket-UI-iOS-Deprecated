...
- (void)setMessageModel:(RPRedpacketModel *)messageModel {
    ...
    self.nameLable.text = [NSString stringWithFormat:@"给 %@ 的红包",messageModel.receiver.userName];
    self.describeLable.text = messageModel.greeting;
    self.moneyLabel.text = [NSString stringWithFormat:@"￥ %@",messageModel.money];
    BOOL isSender = messageModel.isSender;
    NSString *prompt = @"成功领取，已入账至绑定的支付宝账户";
    
    switch (messageModel.statusType) {
