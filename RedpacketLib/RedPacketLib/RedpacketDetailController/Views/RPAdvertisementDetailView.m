...
- (void)setDetailModel:(RPRedpacketModel *)detailModel {
    
    _detailModel = detailModel;
    if (!detailModel.adverInfo) return;
    
    [self.titleLable setText:[NSString stringWithFormat:@"%@的红包",detailModel.sender.userName]];
    [self.subTitleLable setText:detailModel.greeting];
...
- (void)updateConstraints {
    ...
    UIImage * sendButtonBackgroundColorImage = [RedpacketColorStore rp_createImageWithColor:[UIColor colorWithRed:210/255.0
                                                                                                            green:79/255.0
                                                                                                             blue:68/255.0
                                                                                                            alpha:1]];
    ...
    [self.sendButton setBackgroundImage:sendButtonBackgroundColorImage
    ...
    switch (self.detailModel.statusType) {
        ...
        case RPRedpacketStatusTypeCanGrab:{
            ...
            if ([self.rpDelegate respondsToSelector:@selector(advertisementRedPacketAction:)]) {
                ...
                self.detailModel.adverInfo.AdvertisementActionType = RedpacketAdvertisementReceive;
                [self.rpDelegate advertisementRedPacketAction:self.detailModel.adverInfo];
            }
        ...
        case RPRedpacketStatusTypeGrabFinish:
            ...
            if (self.detailModel.adverInfo.shareURLString.length > 0) {
                ...
                if ([self.rpDelegate respondsToSelector:@selector(advertisementRedPacketAction:)]) {
                    ...
                    self.detailModel.adverInfo.AdvertisementActionType = RedpacketAdvertisementAction;
                    [self.rpDelegate advertisementRedPacketAction:self.detailModel.adverInfo];
...
- (void)shareAction {
    ...
    if (!self.alert && (self.detailModel.adverInfo.shareMessage.length ||
                        self.detailModel.adverInfo.shareURLString.length)) {
        
        self.alert = [[UIAlertView alloc] initWithTitle:@""
                                                message:self.detailModel.adverInfo.shareMessage
                                               delegate:self
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    if (buttonIndex) {
        ...
        if ([weakSelf.rpDelegate respondsToSelector:@selector(advertisementRedPacketAction:)]) {
            ...
            self.detailModel.adverInfo.AdvertisementActionType = RedpacketAdvertisementShare;
            [weakSelf.rpDelegate advertisementRedPacketAction:weakSelf.detailModel.adverInfo];
...
- (void)setDetailModel:(RPRedpacketModel *)detailModel{
    ...
    [self.iconImageView rp_setImageWithURL:[NSURL URLWithString:detailModel.adverInfo.logoURLString]];
    ...
    self.backgroundColor = [RedpacketColorStore colorWithHexString:detailModel.adverInfo.colorString
                                                             alpha:1];
    
    [self.bannerImageView rp_setImageWithURL:rpURL(detailModel.adverInfo.bannerURLString)
