...
- (void)setBrifUserInfo:(RPBriefUserInfo *)brifUserInfo
{
    _brifUserInfo = brifUserInfo;
    ...
    self.moneyLabel.text = [NSString stringWithFormat:@"%@元",brifUserInfo.takenMoney];
    ...
    NSString *redpacketTIme = brifUserInfo.takenDate;
    
    if (redpacketTIme.length > 18) {
        
        redpacketTIme = [brifUserInfo.takenDate substringWithRange:NSMakeRange(0, 10)];
        
        if ([locationString isEqualToString:redpacketTIme]) {
            
            // 今天的红包
            self.timeLabel.text = [brifUserInfo.takenDate substringWithRange:NSMakeRange(11, 5)];
            
        }else if(![[locationString substringWithRange:NSMakeRange(0, 4)] isEqualToString:[redpacketTIme substringWithRange:NSMakeRange(0, 4)]]){
            
            // 往年的红包
            self.timeLabel.text = [brifUserInfo.takenDate substringWithRange:NSMakeRange(0, 10)];
            
        }else{
            
            // 昨天和今年之内的红包
            self.timeLabel.text = [brifUserInfo.takenDate substringWithRange:NSMakeRange(5, 11)];
