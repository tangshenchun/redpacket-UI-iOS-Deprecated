...
- (void)setMessageModel:(RPRedpacketModel *)messageModel {
    ...
    if (messageModel.statusType == RPRedpacketStatusTypeCanGrab){
        
        if ([[RPRedpacketAliauth redpacketCurrentUser].userID isEqualToString:messageModel.receiver.userID]) {
            
            self.senderNickNameLable.text = [self nameStringWith:messageModel.sender.userName];
            ...
            NSString *string = [NSString stringWithFormat:@"给%@发了一个红包",[self nameStringWith:messageModel.receiver.userName]];
    ...
    }else if (messageModel.statusType == RPRedpacketStatusTypeGrabFinish){
        
        self.senderNickNameLable.text = [self nameStringWith:messageModel.sender.userName];
        
        NSString *string = [NSString stringWithFormat:@"给%@发了一个红包",messageModel.receiver.userName];
        NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:string];
        [str addAttribute:NSForegroundColorAttributeName value:[UIColor whiteColor] range:NSMakeRange(0,1)];
        [str addAttribute:NSForegroundColorAttributeName value:[UIColor whiteColor] range:NSMakeRange(string.length - 6,6)];
        [str addAttribute:NSForegroundColorAttributeName value:[RedpacketColorStore rp_textcolorYellow] range:NSMakeRange(1,string.length - 7)];
